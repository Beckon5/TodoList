import React,{useState, useEffect} from "react"

const Mark = (props) => {
  const {toDos, text, checkTodo, deleteTodo, item } = props;
  const [checked, setChecked] = useState(toDos[item].checked);
  useEffect(()=>{
    setChecked(toDos[item].checked);
    console.log(toDos[item])
  },[toDos[item].checked])
 return (

    <div className="todo-main__list">

      <input  className="check" type="checkbox" checked={checked} onChange={()=>{ checkTodo(); setChecked(toDos[item].checked) }} />
      <p style={checked ? { textDecoration: "line-through" } : { textDecoration: "none" }} className="text">{text}</p>
      <button className="close" onClick={()=>deleteTodo()}>X</button>
    </div>

  );
}

export default Mark;